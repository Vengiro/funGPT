...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.flash_attention = config.flash_attention if hasattr(config, 'flash_attention') else False
    ...
    def forward(self, x):
        ...
        if self.flash_attention:
            # Flash attention is a recent and  more efficient implementation of the attention mechanism
            attention_output = torch.nn.functional.scaled_dot_product_attention(query=Q, key=K, value=V, dropout_p=self.dropout_attn.p, is_causal=True)
